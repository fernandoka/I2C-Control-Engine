...
divisor_frec: one_hundred_K_counter 
    port map ( clk => Bus2IP_Clk,
           rst =>not Bus2IP_Reset ,
			  speed => motor_ctl(16), --La velocidad a la que va viene en el bit 16. Tiene 2 velocidades
        ...
        motor_ctl(0 to 7) <= motor_inic;
		  motor_ctl (8 to 31) <= (others => '0');
			...
			if (internal_stop='0') then
				if (motor_ce='1') then
					if (motor_ctl(4 to 15)=("000000000000")) then
						motor_ctl(1) <= '1'; -- se detiene el motor
					else
						motor_ctl(4 to 15) <= motor_ctl(4 to 15) - 1; -- decrementa n_steps
					end if;
				end if;
			end if;	
